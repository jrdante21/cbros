	...
	<div class="uk-flex uk-flex-between">
		<div class="uk-flex uk-flex-bottom uk-margin-bottom">
			...
			<div>
				...
				<button class="uk-button uk-button-small uk-button-primary" uk-toggle="#modalAdvance">Add Advance</button>
			</div>
		</div>
...
<div id="modalAdvance" uk-modal>
    <div class="uk-modal-dialog uk-modal-body">
        <h2 class="uk-modal-title">Add Advance</h2>
        <p>
    		<label>Amount</label>
        ...
        <p class="uk-text-right">
            ...
            <button class="uk-button uk-button-primary" id="btnAddAdvance" type="button">Save</button>
        </p>
    </div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#btnAddAdvance").click(function(){
			...
			$("#btnAddAdvance").attr("disabled","disabled").attr("uk-spinner","ratio:1");
			$.post({
				url: "<?=site_url('CBROS/ajax_requests/add_advance/')?>",
				data: data,
				success: function (res) {
					$("#btnAddAdvance").removeAttr("disabled").removeAttr("uk-spinner");
					var r = JSON.parse(res);
					UIkit.notification({  message: r.msg, status: r.stat });

					if (r.stat == 'success') {
						UIkit.modal("#modalAdvance").hide();
						get_records();
					}
				}
			});
	...
	function get_records () {
		$.post({
			url: "<?=site_url('CBROS/ajax_requests/get_advances/')?>"+row,
			...
			success: function (res) {
				...
				$.each(r.data, function(k,v){
					tr += `<tr>
							...
							<td align="right">
								<button class="uk-button uk-button-text uk-text-danger" onclick="delete_data(`+v.advance_id+`)">[remove]</button>
							</td>
						</tr>`;
				});
	...
	function delete_data (id) {
		...
		$.post({
			url: "<?=site_url('CBROS/ajax_requests/delete_advance/')?>",
